...
class CFFIToPyiConverter:
    ...
    def parse_c_code(self, c_code: str) -> None:
        ...
        try:
            ...
            # Try to extract what we can with regex fallbacks
            self._fallback_parsing(c_code)
    ...
    def _process_struct_union_standalone(self, node: Union[c_ast.Struct, c_ast.Union]) -> None:
        ...
        if not node.name or not node.decls:
            return
    ...
    def _get_type_annotation(self, type_node: c_ast.Node) -> str:
        ...
        if isinstance(type_node, c_ast.IdentifierType):
            ...
            return self.c_to_python_types.get(c_type, c_type if c_type not in self.c_to_python_types else 'Any')
        
        elif isinstance(type_node, c_ast.PtrDecl):
            # Pointer types
            pointed_type = self._get_type_annotation(type_node.type)
            if pointed_type == 'int' and isinstance(type_node.type, c_ast.TypeDecl) and isinstance(type_node.type.type, c_ast.IdentifierType):
    ...
    def _fallback_parsing(self, c_code: str) -> None:
        """Fallback parsing using regex when pycparser fails."""
        # Extract function declarations
        func_pattern = r'(\w+)\s*\*?\s*(\w+)\s*\([^)]*\)\s*;'
        for match in re.finditer(func_pattern, c_code):
            return_type = self.c_to_python_types.get(match.group(1), 'Any')
            func_name = match.group(2)
            self.functions.append((func_name, return_type, []))
        
        # Extract struct declarations
        struct_pattern = r'typedef\s+struct\s*\w*\s*\{[^}]+\}\s*(\w+)\s*;'
        for match in re.finditer(struct_pattern, c_code):
            struct_name = match.group(1)
            self.structs[struct_name] = [('field', 'Any')]  # Placeholder
    ...
    def generate_pyi(self) -> str:
        ...
        lines.append('"""')
        lines.append('')
        lines.append('from typing import Any, Optional, Union')
