...
from typing import Any, Dict, List, Optional, Self, Union, TYPE_CHECKING
...
class DispositionType(IntEnum):
    ...
    @classmethod
    def or_int(cls, i: int) -> Union[int, Self]:
...
class RemoteDisposition(Disposition):
    ...
    def __new__(cls, delivery_impl):
        ...
        if state == 0:
            ...
            return super().__new__(RemoteGeneralDisposition)


class RemoteGeneralDisposition(RemoteDisposition):
    ...
    def apply_to(self, local_disposition: 'LocalDisposition'):
        GeneralDisposition(self._type, self._data).apply_to(local_disposition)
...
class GeneralDisposition(LocalDisposition):
