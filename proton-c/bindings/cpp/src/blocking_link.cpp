...
void blocking_link::open(proton::link l) {
    link_ = l;
    connection_.impl_->wait(link_opened(link_.object_), "opening link " + link_.name());
    check_closed();
}
...
void blocking_link::close() {
    link_.close();
    link_not_open link_not_open(link_.object_);
