...
static int pni_encoder_exit_null(void *ctx, pn_data_t *data, pni_node_t *node)
{
  return 0;
}
...
ssize_t pn_encoder_encode_buffer(pn_encoder_t *encoder, pn_data_t *src, pn_buffer_t *buffer)
{
  pn_rwbytes_t bytes = pn_buffer_memory(buffer);
...
ssize_t pn_encoder_size(pn_encoder_t *encoder, pn_data_t *src)
{
  ...
  int err = pni_data_traverse(src, pni_encoder_enter_size, pni_encoder_exit_null, encoder);
