...
TEST_CASE("data_decode_deep_nesting_no_stack_overflow") {
  auto_free<pn_data_t, pn_data_free> data(pn_data(0));

  // Depth far beyond any historical fixed recursion-depth heuristic (previously
  // capped at 50), but comfortably under the absolute PNI_NID_MAX node-id ceiling
  // (2^16 - 1), so the only limit in play is the (disabled) node-count limit below.
  // The iterative decoder must handle this via its own heap-backed node storage
  // rather than recursing on the C stack once per nesting level.
...
TEST_CASE("data_decode_rejects_deeply_nested_values_on_node_limit") {
  ...
  // must fail cleanly with PN_OUT_OF_MEMORY rather than crash or succeed, proving
  // that node count -- not stack depth -- is what now bounds nesting.
  const uint32_t depth = 20000;
  std::vector<uint8_t> buf = build_nested_list32(depth);
...
TEST_CASE("data_decode_rejects_element_count_beyond_node_limit") {
  ...
  // could ever hold. That must be rejected, not silently truncated.
