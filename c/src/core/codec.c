...
int pni_inspect_enter(void *ctx, pn_data_t *data, pni_node_t *node)
{
  ...
  case PN_ARRAY:
    ...
    pn_fixed_string_addf(str, "@%s[", pn_type_name(node->u.array_type));
    return 0;
  case PN_LIST:
    pn_fixed_string_addf(str, "[");
    return 0;
  ...
  default: {
    ...
    case PN_UUID:
      ...
      break;
    default:
      memcpy(&tmp.u, &node->u, sizeof(tmp.u));
      break;
    }
...
void pn_data_set_decode_limits(pn_data_t *data, size_t max_nodes, size_t max_buf)
{
  ...
  /* If max_nid is being lowered below the current capacity, shrink the
   * backing allocation now so the memory is returned to the allocator
   * immediately rather than sitting idle.
   * Target = max(live nodes, max_nid): never evict live nodes.
   * A realloc-to-smaller can theoretically fail on some platforms; if it
   * does we simply keep the oversized buffer — the limit is still enforced
   * by the size >= max_nid check in pni_data_new(), so no correctness
   * problem arises. */
...
static ssize_t pni_data_intern(pn_data_t *data, const char *start, size_t size)
{
  ...
  if (data->max_buf_size > 0 && offset + size + 1 > data->max_buf_size) return PN_OUT_OF_MEMORY;
    ...
    case 'T':                   /* Set type of open array */
      {
        ...
        if (parent->type == PN_ARRAY) {
          parent->u.array_type = (pn_type_t) va_arg(ap, int);
        } else {
...
void pni_data_set_array_type(pn_data_t *data, pn_type_t type)
{
  ...
  if (array) array->u.array_type = type;
}
