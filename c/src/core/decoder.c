 ...
 * per level would let malformed or hostile input overflow the C stack. The
 * state that a recursive decoder would keep in its stack frames - how many
 * children of the enclosing container are still to come, and, for an array,
 * the constructor its elements share - is instead kept in the container's own
 * node, in the scratch space the encoder also uses (pni_decoder_state()).
 *
