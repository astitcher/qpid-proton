...
static inline void pni_emitter_writef128(pni_emitter_t* emitter, char *value) {
...
static inline void emit_symbol(pni_emitter_t* emitter, pni_compound_context* compound, const char* symbol) {
  ...
  if (symbol == NULL) {
    ...
    if (size < 256) {
      ...
      pni_emitter_writev8(emitter, (pn_bytes_t){.size = size, .start = symbol});
    } else {
      pni_emitter_writef8(emitter, PNE_SYM32);
      pni_emitter_writev32(emitter, (pn_bytes_t){.size = size, .start = symbol});
    }
    compound->count++;
  }
}
...
static inline void emit_string(pni_emitter_t* emitter, pni_compound_context* compound, const char* string) {
  ...
  if (string == NULL) {
    ...
    if (size < 256) {
      ...
      pni_emitter_writev8(emitter, (pn_bytes_t){.size = size, .start = string});
    } else {
      pni_emitter_writef8(emitter, PNE_STR32_UTF8);
      pni_emitter_writev32(emitter, (pn_bytes_t){.size = size, .start = string});
    }
    compound->count++;
  }
}
...
static inline void emit_binarynonull(pni_emitter_t* emitter, pni_compound_context* compound, size_t size, const char* bytes) {
  if (size < 256) {
    ...
    pni_emitter_writev8(emitter, (pn_bytes_t){.size = size, .start = bytes});
  } else {
    pni_emitter_writef8(emitter, PNE_VBIN32);
    pni_emitter_writev32(emitter, (pn_bytes_t){.size = size, .start = bytes});
  }
  compound->count++;
}
...
static inline void emit_described_list_copy(pni_emitter_t* emitter, pni_compound_context* compound, uint64_t descriptor, pn_data_t* data) {
