...
static inline void emit_condition(pni_emitter_t* emitter, pni_compound_context* compound, pn_condition_t* condition) {
  if (!condition || !condition->name || !pn_string_get(condition->name)) {
    emit_null(emitter, compound);
    return;
  }

  emit_descriptor(emitter, compound, ERROR);
...
static inline void emit_disposition(pni_emitter_t* emitter, pni_compound_context* compound, pn_disposition_t *disposition)
{
  ...
  emit_descriptor(emitter, compound, disposition->type);
  switch (disposition->type) {
    ...
    default:
      if (disposition->data) {
        emit_copy(emitter, compound, disposition->data);
      } else {
        emit_raw(emitter, compound, disposition->data_raw);
      }
      return;
  }
}
