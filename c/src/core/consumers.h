...
static inline bool pni_consumer_readf128(pni_consumer_t *consumer, void *dst)
{
  ...
  memmove(dst, &consumer->output_start[consumer->position], 16);
  consumer->position += 16;
  return true;
}

static inline bool pni_consume_expected_ubyte(pni_consumer_t* consumer, uint8_t expected)
{
    uint8_t e;
    return pni_consumer_readf8(consumer, &e) && e==expected;
}

static inline bool pni_consume_ulong(pni_consumer_t* consumer, uint64_t *ulong) {
  uint8_t type;
...
static inline bool pni_consume_descriptor(pni_consumer_t* consumer, uint64_t *descriptor) {
  return
    pni_consume_expected_ubyte(consumer, PNE_DESCRIPTOR) &&
