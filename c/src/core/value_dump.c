...
void pn_value_dump_nondescribed_value(uint8_t type, pn_bytes_t value, pn_fixed_string_t *output){
  // Once the output buffer is full there's nothing left to write, so stop here rather
  // than continuing to recurse into (potentially deeply nested) lists/maps/arrays.
